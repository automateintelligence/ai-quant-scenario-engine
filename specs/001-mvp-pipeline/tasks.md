...
- **Source**: `backtesting/` (data, features, distributions, mc, pricing, strategies, simulation, cli)
...
- [ ] T001 Create project directory structure per plan.md (backtesting/{data,features,distributions,mc,pricing,strategies,simulation,cli}, tests/{unit,integration,contract})
...
- [ ] T008 Create base exception classes in backtesting/exceptions.py (DataSourceError, DistributionFitError, ResourceLimitError, PricingError, EpisodeGenerationError)
- [ ] T009 [P] Implement logging configuration in backtesting/utils/logging.py with structured JSON output
- [ ] T010 [P] Create RunConfig dataclass in backtesting/schema/run_config.py per data-model.md with validation
- [ ] T011 [P] Create OptionSpec dataclass in backtesting/models/options.py per data-model.md FR-016
- [ ] T012 [P] Create StrategyParams dataclass in backtesting/schema/strategy.py per data-model.md
- [ ] T013 Create ReturnDistribution abstract base class in backtesting/interfaces/distribution.py per data-model.md
- [ ] T014 Create Strategy abstract base class in backtesting/interfaces/strategy.py per data-model.md
- [ ] T015 Create OptionPricer abstract base class in backtesting/interfaces/pricing.py per data-model.md
- [ ] T016 Create CandidateSelector abstract base class in backtesting/interfaces/candidate_selector.py per FR-CAND-001
- [ ] T017 [P] Implement resource estimator in backtesting/utils/resources.py (estimate memory footprint for n_paths × n_steps, enforce FR-013/FR-018 thresholds)
- [ ] T018 [P] Create run metadata schema in backtesting/schema/run_meta.py with JSON serialization per FR-008/FR-019
...
- [ ] T021 [P] [US1] Implement YFinanceDataSource adapter in backtesting/data/yfinance.py with load_ohlcv(symbol, start, end, interval) per FR-001
- [ ] T022 [P] [US1] Create SchwabDataSourceStub in backtesting/data/schwab_stub.py with matching interface per FR-011/FR-017
- [ ] T023 [P] [US1] Implement data validation in backtesting/data/validation.py (check OHLCV columns, handle missing data per FR-010)
- [ ] T024 [US1] Create DataSource factory in backtesting/data/factory.py to select yfinance vs schwab per config per FR-009
...
- [ ] T033 [US1] Implement BlackScholesPricer in backtesting/pricing/black_scholes.py with price(spot, spec, ttm) per FR-016 and plan.md reference
- [ ] T034 [P] [US1] Create OptionPricer factory in backtesting/pricing/factory.py to select black_scholes (default) per FR-009/FR-016

#### Strategy Framework (US1)

- [ ] T035 [US1] Create example StockBasicStrategy in backtesting/strategies/stock_basic.py implementing Strategy interface per data-model.md
- [ ] T036 [US1] Create example OptionCallStrategy in backtesting/strategies/option_call.py implementing Strategy interface with OptionSpec per data-model.md
- [ ] T037 [US1] Implement StrategySignals dataclass in backtesting/schema/signals.py (signals_stock, signals_option, option_spec, features_used) per data-model.md

#### Simulation Engine (US1)

- [ ] T038 [US1] Implement MarketSimulator class in backtesting/simulation/simulator.py with simulate_stock() and simulate_option() per FR-003/FR-004 and plan.md reference
- [ ] T039 [US1] Implement metrics calculator in backtesting/simulation/metrics.py (mean/median P&L, drawdown, VaR/CVaR, Sharpe/Sortino) per FR-003
- [ ] T040 [US1] Create SimulationRun orchestrator in backtesting/simulation/run.py to coordinate MC, strategies, simulation per data-model.md
- [ ] T041 [US1] Implement run_compare() function in backtesting/simulation/compare.py integrating all components for stock vs option comparison per FR-005

#### Artifacts & Persistence (US1)

- [ ] T042 [P] [US1] Implement MetricsReport serialization in backtesting/schema/metrics.py with JSON/CSV export per FR-008
- [ ] T043 [P] [US1] Create run artifact writer in backtesting/utils/artifacts.py (metrics, plots, run_meta.json) per FR-008
- [ ] T044 [US1] Implement run_meta.json persistence in backtesting/utils/run_meta.py with all required fields per FR-019 and data-model.md example

#### CLI (US1)

- [ ] T045 [US1] Create Typer CLI entrypoint in backtesting/cli/main.py with compare command per FR-005
- [ ] T046 [US1] Implement config validation in backtesting/cli/validation.py enforcing contracts/openapi.yaml CompareRequest schema
- [ ] T047 [US1] Wire compare CLI to run_compare() in backtesting/cli/commands/compare.py with proper error handling per plan.md error policies
...
- [ ] T049 [P] [US4] Implement technical indicator wrapper in backtesting/features/indicators.py using pandas-ta (SMA, RSI, volume_z) per FR-006
- [ ] T050 [P] [US4] Create feature computation pipeline in backtesting/features/pipeline.py to enrich OHLCV with indicators
- [ ] T051 [US4] Implement gap percentage calculator in backtesting/features/gap.py (open vs prev_close) for selector rules

#### Candidate Selection (US4)

- [ ] T052 [US4] Implement GapVolumeSelector in backtesting/selectors/gap_volume.py extending CandidateSelector per FR-CAND-001/FR-CAND-006 (default MVP selector)
- [ ] T053 [US4] Implement selector.score() and selector.select() methods per data-model.md CandidateSelector interface
- [ ] T054 [P] [US4] Create CandidateEpisode dataclass in backtesting/schema/episode.py per data-model.md
- [ ] T055 [US4] Implement build_candidate_episodes() in backtesting/selectors/episodes.py per data-model.md reference code

#### Screening Engine (US4)

- [ ] T056 [US4] Implement universe screening logic in backtesting/simulation/screen.py to scan symbols and apply selector per FR-CAND-001
- [ ] T057 [US4] Add parallel symbol processing with max_workers cap per FR-018 concurrency model
- [ ] T058 [US4] Implement candidate ranking and top_n filtering in backtesting/simulation/screen.py per contracts/openapi.yaml ScreenRequest
- [ ] T059 [US4] Add error handling for missing/partial data per symbol per spec.md US4 acceptance scenario 2

#### CLI (US4)

- [ ] T060 [US4] Create screen command in backtesting/cli/commands/screen.py with universe and selector arguments per contracts/openapi.yaml ScreenRequest
...
- [ ] T063 [US5] Implement conditional episode filtering in backtesting/simulation/conditional.py to extract candidate-only windows per FR-CAND-003
- [ ] T064 [US5] Extend MarketSimulator to support episode-level P&L tracking in backtesting/simulation/simulator.py
- [ ] T065 [US5] Implement episode-level metrics aggregation in backtesting/simulation/metrics.py per spec.md SC-009
- [ ] T066 [US5] Create run_conditional_backtest() in backtesting/simulation/conditional.py orchestrating selector → episodes → strategy evaluation per FR-CAND-003
- [ ] T067 [US5] Add unconditional vs conditional metrics comparison reporting per spec.md US5 acceptance scenario 2

#### CLI (US5)

- [ ] T068 [US5] Create conditional command in backtesting/cli/commands/conditional.py with selector parameter per contracts/openapi.yaml ConditionalRequest
...
- [ ] T071 [US2] Implement grid parameter expansion in backtesting/simulation/grid.py to generate StrategyParams combinations per contracts/openapi.yaml StrategyGridConfig
- [ ] T072 [US2] Create grid runner with ProcessPoolExecutor in backtesting/simulation/grid.py using max_workers from config per plan.md concurrency model
- [ ] T073 [US2] Implement per-config metrics collection and aggregation in backtesting/simulation/grid.py
- [ ] T074 [US2] Add objective function scoring and ranking in backtesting/simulation/metrics.py per FR-007 and spec.md US2 acceptance scenario 1
- [ ] T075 [US2] Implement resource limit preflight check in backtesting/simulation/grid.py per FR-018 (estimate time/memory for N configs, abort if > thresholds)
- [ ] T076 [US2] Add race condition prevention and output integrity validation per spec.md US2 acceptance scenario 2

#### CLI (US2)

- [ ] T077 [US2] Create grid command in backtesting/cli/commands/grid.py with grid parameter per contracts/openapi.yaml GridRequest
...
- [ ] T084 [US6] Create run_conditional_mc() in backtesting/simulation/conditional_mc.py integrating conditional sampling with strategies per FR-CAND-004

#### CLI (US6)

- [ ] T085 [US6] Extend conditional CLI to support monte_carlo mode in backtesting/cli/commands/conditional.py
...
- [ ] T088 [US7] Create YAML configuration schema in backtesting/config/schema.py matching contracts/openapi.yaml request schemas
- [ ] T089 [P] [US7] Implement YAML config loader in backtesting/config/loader.py with validation
- [ ] T090 [P] [US7] Add environment variable override support in backtesting/config/env.py per plan.md onboarding
...
- [ ] T094 [P] [US7] Add optional PyVollibPricer in backtesting/pricing/py_vollib.py per FR-016 advanced toggle
- [ ] T095 [P] [US7] Create QuantLib pricer stub in backtesting/pricing/quantlib_stub.py for future extension per FR-016
...
- [ ] T099 [US3] Implement dynamic indicator registry in backtesting/features/registry.py to declare indicators via config per FR-006
- [ ] T100 [US3] Create indicator definition schema in backtesting/schema/indicators.py supporting pandas-ta function specs
- [ ] T101 [US3] Extend feature pipeline to apply registered indicators from config per spec.md US3 acceptance scenario 1
- [ ] T102 [P] [US3] Implement macro series loader in backtesting/data/macro.py with alignment logic per FR-014
...
- [ ] T109 [P] [US8] Implement data version fingerprinting in backtesting/data/versioning.py to detect Parquet schema/content changes per FR-019
...
- [ ] T111 [US8] Implement replay mode in backtesting/simulation/replay.py to reload run_meta and regenerate paths per FR-019 and spec.md US8 acceptance scenario 2
...
- [ ] T115 [US8] Create replay command in backtesting/cli/commands/replay.py per contracts/openapi.yaml ReplayRequest
...
- [ ] T120 [P] Implement optional plotly report generation in backtesting/utils/plots.py per FR-008
- [ ] T121 [P] Add quantstats integration for tearsheet generation in backtesting/utils/quantstats_report.py per plan.md dependencies
- [ ] T122 Create performance profiling utilities in backtesting/utils/profiling.py to validate SC-001/SC-002/SC-003 time budgets
...
Task: "Implement YFinanceDataSource in backtesting/data/yfinance.py"
Task: "Create SchwabDataSourceStub in backtesting/data/schwab_stub.py"
Task: "Implement data validation in backtesting/data/validation.py"

# Then sequential:
Task: "Create DataSource factory in backtesting/data/factory.py"
```
...
- File paths follow plan.md structure: backtesting/{module}/file.py
